...
func TestSpaceSaving(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := New(tt.args.epsilon, tt.args.k)
			for _, v := range tt.args.data {
				s.Incr(v)
			}
			if got := s.TopK(); !IsSubset(got, tt.want) {
				t.Errorf("SpaceSaving() = %v, want %v", got, tt.want)
			}
			t.Logf("SpaceSaving() = %v, want %v", s.TopK(), tt.want)
		})
	}
}
...
func Benchmark_Spacesaving_Incr_ε0_001(b *testing.B) {
	summary := New(0.001, 50)
	for i := 0; i < b.N; i++ {
